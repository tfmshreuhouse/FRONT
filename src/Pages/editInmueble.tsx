...
function EditInmueble() {
    ...
    }, [Direccion, tipoInmueble, selectedCountry, selectedCity, Pisos, Habitaciones, BanosCom, BanosMed, Cocina, Lavado, Patio, Balcon, Estacionamiento, Elevador, Piscina, AreaPublicas, Fumar, Mascotas, Reuniones, Descripcion, Indicaciones, Images]);
    ...
    useEffect(() => {
        const obtenerDatosTipoInmueble = async () => {
            ...
            const idInmuebleParam = segments[segments.length - 1];
            
            if (idInmuebleParam) {
                const idInmuebleValue = parseInt(idInmuebleParam);
                idInmueble = idInmuebleValue;
                ...
                try {
                    ...
                    const response = await axios.get<{ success: boolean; data: any }>(
                        "http://localhost:3000/rest/inmuebles/"+ idInmuebleValue,
                        {
                            headers: {
                    ...
                    setTipoInmueble({ id: response.data.data.TiposInmueble.id, tipo: response.data.data.TiposInmueble.tipo });
    ...
    const GuardarInmueble = async (e: any) => {
        ...
        const FormDataInmueble = {
            ...
            Ciudad: selectedCity,
            Direccion: Direccion,
            TiposInmuebleId: tipoInmueble?.id,
        ...
        try {
            ...
            idInmueble = parseInt(response.data.data.id);
        } catch (error) {
    ...
    if (e.target.files) {
        ...
        reader.onload = (event) => {
            ...
            previews.push(event.target.result.toString());
        ...
        <>
            <div>
                <form onSubmit={GuardarInmueble}>
                    ...
                    <div>
                        <div>
                            <Button label="Seleccionar archivos" icon="pi pi-upload" onClick={handleFileSelect} />
                    ...
                    <Button type="submit" label="Guardar" disabled={!formCompleted}/>
                </form>
            </div>
        </>
    );
